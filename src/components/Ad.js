...
const Ad = (props) => {
  ...
  const handleSubmitBid = (e) => {
    e.preventDefault();
    // Place bid
    props.placeBid(props.adDetails._id, bidPrice).then(() => {
  ...
  return (
    <Container maxWidth="md" sx={{ p: "16px !important" }}>
      {props.loading ? (
        <LoadingDisplay />
      ) : (
        <Fragment>
                      ...
                      {auctionStatus() === 'Sold' ? <>
                        {props.highestBid.user._id === props.auth.user._id ?
                          <Button variant="contained" onClick={() => {
                            navigate('/payment')
                          }}>Pay now</Button> : <Button disabled variant="contained">Already Sold</Button>
                        }
                      </> : <Box sx={bidContainer}>
                        <TextField
                          label="Price in Rs"
